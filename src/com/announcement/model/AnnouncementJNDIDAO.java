...
public class AnnouncementJNDIDAO implements AnnouncementDAO_interface {
	...
	private static final String INSERT = "INSERT INTO ANNOUNCEMENT (ANC_NO,ANC_CON,BACKSTAGE_NO,CREATED_AT,UPDATED_AT,ACTIVE)"
			+ "VALUES (ANNOUNCEMENT_SEQ.nextval,?,?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,?)";
	private static final String UPDATE = "UPDATE ANNOUNCEMENT " + "ANC_CON=?," + "UPDATED_AT=?," + "ACTIVE=? "
			+ "WHERE ANC_NO=?";
	private static final String DELETE = "DELETE FORM ANNOUNCENMENT WHERE ACN_NO=?";
	private static final String GET_ALL = "SELECT * FROM ANNOUNCEMENT ORDER BY ANC_NO DESC";
	...
	public void update(AnnouncementVO ancmod) {
		...
		try {
			...
			pstmt.setString(1, ancmod.getAnc_no());
			pstmt.setString(2, ancmod.getBackstage_no());
			pstmt.setTimestamp(3, ancmod.getUpdated_at());
			pstmt.setInt(4, ancmod.getActive());
